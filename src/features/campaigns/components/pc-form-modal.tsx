...
import { updatePc } from "../api/update-pc";
...
import type { PcSummary, PcDetail } from "@/features/pcs/types/pc";
...
export function PcFormModal({ campaignId, onClose, onSuccess, initialPc, pcId }: PcFormModalProps) {
  const [name, setName] = useState(initialPc?.name ?? "");
  const [description, setDescription] = useState(initialPc?.description ?? "");
  const [tagline, setTagline] = useState(initialPc?.tagline ?? "");
  const [pronouns, setPronouns] = useState(initialPc?.pronouns ?? "");
  const [origin, setOrigin] = useState(initialPc?.origin ?? "");
  const [identity, setIdentity] = useState(initialPc?.identity ?? "");
  const [theme, setTheme] = useState(initialPc?.theme ?? "");
  const [money, setMoney] = useState(initialPc?.money != null ? String(initialPc.money) : "0");
  const [dexDie, setDexDie] = useState<AttributeDie>((initialPc?.dexterity_die as AttributeDie | null) ?? "d8");
  const [insDie, setInsDie] = useState<AttributeDie>((initialPc?.insight_die as AttributeDie | null) ?? "d8");
  const [mgtDie, setMgtDie] = useState<AttributeDie>((initialPc?.might_die as AttributeDie | null) ?? "d8");
  const [wilDie, setWilDie] = useState<AttributeDie>((initialPc?.willpower_die as AttributeDie | null) ?? "d8");
  ...
  const [equipment, setEquipment] = useState<{ main_hand: number | null; off_hand: number | null; armor: number | null; accessory: number | null }>({
    main_hand: null, off_hand: null, armor: null, accessory: null,
  });
  ...
  const { imgUrl, uploading: uploadingImage, imageFieldProps } = useFormImageUpload(campaignId, "pc", initialPc?.img_key ?? null);

  const [levelsInitialized, setLevelsInitialized] = useState(!initialPc);
  ...
  useEffect(() => {
    if (!initialPc || levelsInitialized || campaignJobs.length === 0) return;

    const levels: PcLevelEntry[] = [];
    for (const job of initialPc.jobs) {
      for (let i = 0; i < job.level; i++) {
        levels.push({ job_id: job.id, power_id: null, spell_id: null });
      }
    }

    const commonPowers = initialPc.powers.filter((p) => p.type !== "heroic");
    let levelIdx = 0;
    for (const power of commonPowers) {
      for (let i = 0; i < power.level; i++) {
        if (levelIdx < levels.length) {
          levels[levelIdx].power_id = power.id;
          levelIdx++;
        }
      }
    }

    const heroicByJob: Record<number, number | null> = {};
    const heroicPowers = initialPc.powers.filter((p) => p.type === "heroic");
    for (const job of initialPc.jobs) {
      const jobCatalogItem = campaignJobs.find((j) => j.id === job.id);
      if (!jobCatalogItem) continue;
      const heroicPower = heroicPowers.find((hp) => {
        const publicPower = campaignPowers.find((p) => p.id === hp.id);
        return publicPower && publicPower.job_name.includes(jobCatalogItem.name);
      });
      heroicByJob[job.id] = heroicPower?.id ?? null;
    }

    setPcLevels(levels);
    setHeroicPowerByJobId(heroicByJob);

    if (initialPc.equipment) {
      setEquipmentEnabled(true);
      setEquipment({
        main_hand: initialPc.equipment.main_hand?.id ?? null,
        off_hand: initialPc.equipment.off_hand?.id ?? null,
        armor: initialPc.equipment.armor?.id ?? null,
        accessory: initialPc.equipment.accessory?.id ?? null,
      });
    }

    setInventory(
      initialPc.inventories.map((i) => ({
        item_id: i.item.id,
    ...
    setBonds(
      initialPc.bonds.map((b) => ({
        target_type: b.target_type as BondTargetType,
        target_id: b.target_id,
        target_name: b.target_name ?? "",
        admiration_axis: (b.admiration_axis as AdmirationAxis | null) ?? "",
        loyalty_axis: (b.loyalty_axis as LoyaltyAxis | null) ?? "",
        affection_axis: (b.affection_axis as AffectionAxis | null) ?? "",
        description: b.description ?? "",
      }))
    );

    setLevelsInitialized(true);
  }, [initialPc, levelsInitialized, campaignJobs, campaignPowers]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (initialPc && pcId) {
        await updatePc(campaignId, pcId, payload);
      } else {
        await createPc(campaignId, payload);
      }
      onSuccess();
    } catch (err) {
  ...
  const title = initialPc ? "Editar personagem (PC)" : "Novo personagem (PC)";

  return (
    <FormModal title={title} onClose={onClose}>
      <form onSubmit={handleSubmit} noValidate>
        {error ? <p className="manage-form__error">{error}</p> : null}
        {initialPc && !levelsInitialized && <p className="manage-form__hint">Carregando dados do personagem...</p>}
        ...
        <div className="manage-form__actions">
          <Button type="submit" variant="primary" disabled={submitting || uploadingImage || !canSubmit}>
            {initialPc ? (submitting ? "Salvando..." : "Salvar") : (submitting ? "Criando..." : "Criar personagem")}
          </Button>
